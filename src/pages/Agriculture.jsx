  ...
  return (
    <div className="container" style={{ padding: '2.5rem 1.25rem' }}>
            ...
            <div key={field.id} className="card card-interactive" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h3 style={{ fontSize: '1.4rem', fontWeight: '700', color: 'var(--color-primary-700)' }}>
                    {field.name}
                  </h3>
                  <span className="badge badge-green">সচল মাঠ</span>
                </div>

                <div style={{ fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '0.6rem', color: 'var(--text-main)' }}>
              ...
              <div style={{ marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>আপডেট: {field.updatedAt}</span>
                <Link to={`/agriculture/${field.id}`} className="btn btn-primary btn-sm">
                  বিস্তারিত দেখুন →
                </Link>
              </div>
            </div>
      ...
      {activeTab === 'machines' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: '700' }}>পাওয়ার টিলার, সেচ পাম্প ও কম্বাইন হারভেস্টার</h2>
            {currentUser ? (
              <button onClick={() => setShowMachineModal(true)} className="btn btn-primary">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
            {machines.map(mach => (
              <div key={mach.id} className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <img src={mach.image} alt={mach.name} style={{ width: '100%', height: '180px', objectFit: 'cover', borderRadius: 'var(--radius-md)', marginBottom: '1rem' }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <span className="badge badge-amber">{mach.category}</span>
                    <span style={{ fontWeight: '700', color: 'var(--color-primary-700)', fontSize: '1.1rem' }}>
                      ৳{mach.price} <span style={{ fontSize: '0.8rem', fontWeight: '400', color: 'var(--text-muted)' }}>/{mach.priceUnit}</span>
                    </span>
                  </div>

                  <h3 style={{ fontSize: '1.15rem', fontWeight: '700', marginBottom: '0.5rem' }}>{mach.name}</h3>
                  <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginBottom: '0.75rem', lineHeight: '1.5' }}>
                    {mach.description}
                  </p>

                  <div style={{ fontSize: '0.85rem', display: 'flex', flexDirection: 'column', gap: '0.35rem', color: 'var(--text-main)', backgroundColor: 'var(--bg-elevated)', padding: '0.75rem', borderRadius: 'var(--radius-sm)' }}>
      ...
      {activeTab === 'leases' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: '700' }}>জমি লিজের বর্তমান বিজ্ঞাপনসমূহ</h2>
            {currentUser ? (
              <button onClick={() => setShowLeaseModal(true)} className="btn btn-primary">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
            {leases.map(lease => (
              <div key={lease.id} className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <img src={lease.images[0]} alt="জমি" style={{ width: '100%', height: '180px', objectFit: 'cover', borderRadius: 'var(--radius-md)', marginBottom: '1rem' }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <span className="badge badge-green">পরিমাণ: {lease.landAmount}</span>
                    <span style={{ fontWeight: '800', color: 'var(--color-primary-700)', fontSize: '1.2rem' }}>
      ...
      {showMachineModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ maxWidth: '540px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: '1.4rem', marginBottom: '1rem' }}>নতুন কৃষিযন্ত্রের সেবা যোগ করুন</h2>
      ...
      {/* Modal: Create Land Lease */}
      {showLeaseModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ maxWidth: '540px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: '1.4rem', marginBottom: '1rem' }}>নতুন জমি লিজের বিজ্ঞাপন দিন</h2>
